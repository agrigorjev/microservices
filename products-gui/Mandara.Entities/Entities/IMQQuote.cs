...
namespace Mandara.Entities
{
    ...
    public class IMQQuote
    {
        ...
        public virtual IMQMessage Message
        {
            get { return _message; }
            set
            {
        ...
        public bool IsBlue
        {
            ...
            {
                bool result;
                if (IsRed || IsGreen || IsGrey || !MandaraPrice.HasValue || !Mid.HasValue || Mid.Value == 0)
                {
                    result = false;
                }
                else
                {
                    decimal diffMinusOne = Math.Abs(MandaraPrice.Value / Mid.Value) - 1;
                    decimal totalDiff = Math.Abs(diffMinusOne);
                    result = totalDiff > Threshold;
                }
        ...
        public IMQQuote(HALProduct vhalProduct)
        {
            IsVHALProduct = true;
            ParsedProductId = vhalProduct.ProductId;
            Message = new IMQMessage
            {
        ...
        public string GetTextForSpeech(IMQMessage message)
        {
            if (!IsGreen && !IsRed)
                return String.Empty;
