...
namespace Mandara.Business.Managers
{
    ...
    /// </summary>
    public static class AdministrativeAlertManager
    {
        /// <summary>
        ...
        {
            return new MandaraEntities(MandaraEntities.DefaultConnStrName, nameof(AdministrativeAlertManager));
        }
        ...
        public static void SaveGroup(AdministrativeAlertGroup group, AuditContext auditContext)
        {
            ...
            using (var transaction = context.Database.BeginTransaction())
            {
                ...
                context
                    .Save(context.AdministrativeAlertGroups, group, p => p.GroupId, out newIdFunc)
                    .Save(context.AlertPhones, group.Phones, a => a.AlertPhoneID, a => a.AlertGroupId == group.GroupId)
                    .Save(context.AlertVoicePhones, group.VoicePhones, a => a.AlertVoicePhoneID, m => m.AlertGroupId == group.GroupId)
                    .Save(context.AlertEmailAddresses, group.Emails, a => a.AlertEmailAddressID, m => m.AlertGroupId == group.GroupId);

                context.ChangeTracker.DetectChanges();
                context.SaveChanges();

                group.GroupId = newIdFunc();

                var originalGroup = context.AdministrativeAlertGroups.Include("Users").FirstOrDefault(g => g.GroupId == group.GroupId);
                if (originalGroup != null)
                {
                    ...
                    {
                        originalGroup.Users.Add(users.FirstOrDefault(uu => uu.UserId == u.UserId));
                    }
                }
                ...
                transaction.Commit();
                _alertGroups = new BindingList<AdministrativeAlertGroup>(context.AdministrativeAlertGroups.ToList());
            }
        }
        /// <summary>
